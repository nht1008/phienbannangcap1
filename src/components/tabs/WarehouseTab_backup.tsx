...
import { Search, AlertTriangle, Plus, Star } from 'lucide-react';
...
const SmartDisposalTab: React.FC<{
  ...
  const availableProductNames = React.useMemo(() => {
    return [...new Set(inventory.filter(p => p.quantity > 0).map(p => p.name))].filter(Boolean);
  }, [inventory]);
  ...
  const getAvailableOptionsForProduct = (productName: string) => {
    ...
    const availableColors = [...new Set(productsWithSameName.map(p => p.color))].filter(Boolean);
    const availableQualities = [...new Set(productsWithSameName.map(p => p.quality).filter(q => q !== undefined && q !== null))];
    const availableSizes = [...new Set(productsWithSameName.map(p => p.size))].filter(Boolean);
    const availableUnits = [...new Set(productsWithSameName.map(p => p.unit))].filter(Boolean);
    
    // Kiểm tra xem có sản phẩm nào có quality là undefined/null không
    const hasNoQuality = productsWithSameName.some(p => p.quality === undefined || p.quality === null);
  ...
  const handleProductNameSelect = (productName: string) => {
    ...
    setSelectedQuality(options.hasNoQuality ? 'none' : (options.qualities.length > 0 ? options.qualities[0] : ''));
        ...
        >
          ...
          <PopoverContent className="w-[400px] p-0" align="start">
            <Command shouldFilter={false}>
              ...
              />
              <CommandList>
                <ScrollArea className="h-[300px]">                  {localSearchTerm ? (
                    <>
                      <CommandEmpty>Không tìm thấy sản phẩm.</CommandEmpty>
                      <CommandGroup heading="Kết quả tìm kiếm">
                        {filteredResults.map((result) => {
                    ...
                    <CommandGroup heading="Tất cả sản phẩm có sẵn">
                      {inventory.filter(p => p.quantity > 0).slice(0, 20).map((product) => (
                        <CommandItem
                          key={product.id}
                          value={`${product.id}-${product.name}`}
                          onSelect={() => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg text-gray-900">Lịch sử loại bỏ gần đây</CardTitle>
        </CardHeader>
        <CardContent>
